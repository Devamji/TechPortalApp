...
public class CourseDao {
	...
	public boolean saveCourse(Course course) {
		...
		try {
			...
			String query = "insert into course (coursename,courseduration,coursefee,status,cdesc,courseimg,pdfname,userId) values (?,?,?,?,?,?,?,?)";
	...
	public boolean updateCourse(Course course) {
		...
		try {
			...
			String query = "update course set coursename=? , courseduration=?,coursefee=?,status=?,cdesc=?,courseimg=?,pdfname=? where cid=?";
			PreparedStatement ps = conn.prepareStatement(query);
			ps.setString(1, course.getCourseName());
			ps.setString(2, course.getCourseDuration());
			ps.setInt(3, course.getCourseFee());
	...
	public List<Course> getLatestCourses() {
		...
		try {
			...
			String query = "select cid,coursename,coursefee,status,cdesc,courseimg  from course order by cid desc limit 6 ";
			PreparedStatement ps = conn.prepareStatement(query);
			ResultSet rs = ps.executeQuery();
	...
	public List<Course> getAllCourses() {
		...
		try {
			...
			String query = "select cid,coursename,coursefee,status,cdesc,courseimg  from course order by cid desc";
			PreparedStatement ps = conn.prepareStatement(query);
			ResultSet rs = ps.executeQuery();
